...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setView }) => {
  ...
  const renderIcon = (id: string, iconStr: string) => {
    if (id === 'accounts') {
      return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M8 14v20c0 4.418 7.163 8 16 8 8.837 0 16-3.582 16-8V14M8 14c0 4.418 7.163 8 16 8 8.837 0 16-3.582 16-8M8 14c0-4.418 7.163-8 16-8 8.837 0 16 3.582 16 8m0 10v10c0 4.418-7.163 8-16 8-8.837 0-16-3.582-16-8V24" />;
    }
    if (id === 'profile') {
      return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M24 22c5.523 0 10-4.477 10-10S29.523 2 24 2 14 6.477 14 12s4.477 10 10 10zm0 4c-8.837 0-16 7.163-16 16v2h32v-2c0-8.837-7.163-16-16-16z" />;
    }
    if (id === 'admin') {
      return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M34 16c0 5.523-4.477 10-10 10s-10-4.477-10-10 4.477-10 10-10 10 4.477 10 10zm-10 14c-8.837 0-16 7.163-16 16v2h12v-8h8v8h12v-2c0-8.837-7.163-16-16-16z" />;
    }
    return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d={iconStr} />;
  };

  return (
    <>
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#1e1b4b] border-t border-purple-900/50 flex justify-around p-4 z-50 overflow-x-auto">
        {items.map(item => (
          <button
            key={item.id}
            onClick={() => setView(item.id as AppView)}
            className={`flex flex-col items-center gap-1 min-w-[60px] ${activeView === item.id ? 'text-purple-400' : 'text-slate-500'}`}
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 48 48">
                {renderIcon(item.id, item.icon)}
            </svg>
      ...
      <nav className="hidden md:flex flex-col w-64 bg-[#1e1b4b]/50 border-r border-purple-900/30 p-8 h-screen sticky top-0">
        ...
        <div className="mt-auto pt-8">
            <div className="bg-gradient-to-br from-purple-600 to-indigo-700 p-6 rounded-3xl text-white shadow-xl">
                <p className="text-xs font-bold uppercase tracking-widest opacity-70 mb-1">Acesso</p>
                <p className="font-bold text-lg">Premium Ativo</p>
                <div className="mt-4 text-xs bg-black/20 p-2 rounded-lg">Foco em Segurança e Privacidade</div>
            </div>
        </div>
      </nav>
    </>
